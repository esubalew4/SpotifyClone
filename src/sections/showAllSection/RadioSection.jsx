...
const RadioSection = () => {
  ...
  return (
    <div className=" text-text flex gap-2 flex-1 min-h-[430px]">
      <MainLeft />
      <div
        className={`py-20 sm:py-0 sm:mr-2 bg-secondary flex-1 rounded-md min-w-72  overflow-y-auto overflow-x-hidden sm:[&::-webkit-scrollbar]:w-3 sm:[&::-webkit-scrollbar-track]:bg-transparent sm:[&::-webkit-scrollbar-thumb]:bg-primary select-none flex flex-col p-8 gap-4`}
      >
        <p className="text-lg sm:text-4xl font-bold ">Popular Radios</p>
        <div className="flex flex-wrap gap-1">
          {fixedRadio.map((item) => (
            <Link key={item.id} to={`/playlist/${item.id}`}>
              <div className="max-w-40 sm:max-w-[168px] group flex flex-col p-3 cursor-pointer hover:bg-primary rounded-md duration-150 h-fit">
                {/*cover box */}
