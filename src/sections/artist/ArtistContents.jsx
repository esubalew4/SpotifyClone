...
const ArtistContents = ({ artist }) => {
  ...
  return (
    <div
      className={` relative flex-1 rounded-md sm:min-w-[500px]  overflow-y-hidden flex flex-col gap-4 sm:mr-2 mt-12 sm:mt-0 `}
    >
      <div className="absolute z-1 w-full h-[270px] left-0 sm:-left-3">
        {artist.coverBg ? (
          <img
            src={artist.coverBg}
      ...
      <div
        ...
        } absolute w-full h-16 bg-gray-800 shadow-lg shadow-text/5 top-0 sm:-left-3 flex items-center pl-4 sm:pl-6 gap-4 z-3`}
      >
        {/* scroll header */}
      ...
      >
        ...
        >
          ...
          <div className=" font-bold text-sm flex flex-col w-full">
            ...
            <p className="mx-6 text-2xl sm:text-5xl md:text-6xl  text-text-p font-bold w-fit">
              {artist.name}
            </p>
            <div className="flex gap-1 bg-gradient-to-t from-blue-950 to-transparent pt-4 pb-10 px-6">
              <p className="text-text-p">
                <span>{artist.listeners} monthly listeners</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="z-2 befor bg-secondary -mt-32 h-[600px] shrink-0">
          ...
          <div className=" sm:p-12 bg-secondary">
            <p className="mb-4 text-xl">About</p>
            <div className="relative ml-8 sm:ml-0 size-48 sm:size-full rounded-lg overflow-hidden hover:scale-101 duration-300">
              <img
                src={artist.cover}
              ...
              />{" "}
              {artist.score && (
                <div className="absolute top-1 sm:top-10 font-medium right-1 sm:right-6 bg-cyan-600 rounded-full sm:size-24 py-2.5 sm:p-0 px-1 flex-col flex-center text-[16px] text-text-p">
                  <p className="text-2xl font-bold">#{artist.score}</p>
                  <p className="hidden sm:*:block text-[13px]">in the world</p>
                </div>
              )}
              <div className="absolute bottom-2 sm:bottom-10 font-medium left-0 sm:left-1/2 sm:-translate-x-1/2 w-full px-1 sm:px-12 text-[16px] text-text-p">
                <p className="hidden sm:block">
                  <span className="text-lg">{artist.listeners}</span> monthly
                  listeners
                </p>
                <p className=" line-clamp-2 sm:line-clamp-4 mt-3">
                  {" "}
                  {artist.about}
                </p>
              </div>
